...
('Đại Học'),
('Sau Đại Học');
insert into devision (devision_name)
values
('Sale-Marketing'),
('Hành chính'),
('Phục vụ'),
...
insert into service (service_name, service_area, service_costs, service_max_people , rent_type_id , service_type_id ,  standard_room , description_other_convenience , pool_area , number_of_floors)
values
...
insert into employee (employee_name,employee_birthday,employee_id_card,employee_salary,employee_phone,employee_email,employee_address,position_id,education_degree_id,devision_id, user_name)
values
...
drop procedure find_eemployee;
DELIMITER $$
create PROCEDURE find_eemployee( IN eemployee_name varchar(50), IN eemployee_phone varchar(50),IN eemployee_id int)
BEGIN
select * from furama_resort.employee
    ...
    join devision on employee.devision_id = devision.devision_id
    join `user` on employee.user_name = `user`.user_name
    where  employee_name like  concat("%",eemployee_name,"%") or employee_phone like eemployee_phone or employee_id = eemployee_id;
END$$
DELIMITER ;

call find_eemployee("Võ","0374443232",4);
...
call find_employee_by_id(?);


DELIMITER $$
create PROCEDURE update_employee(IN e_id int, IN e_name varchar(50),
 IN birthday varchar(50), IN id_card varchar(50),IN salary varchar(50),IN phone varchar(50),
 IN email varchar(50),IN address varchar(50),IN position_id int,IN edu_id int,IN devision_id int,IN user_name varchar(50))
BEGIN
	UPDATE employee
	SET employee.employee_name = e_name,
    employee.employee_birthday = birthday,
    employee.employee_id_card = id_card,
    ...
    employee.education_degree_id = edu_id,
    employee.devision_id = devision_id,
